...
local function tpItemsGetList(filter)
    ...
    local function checkObj(obj)
        local name = obj.Name
        if (filter == "All" and (name == "BloxyCola" or name == "Medkit"))
        or (filter == "BloxyCola" and name == "BloxyCola")
        or (filter == "Medkit" and name == "Medkit") then
    ...
    -- only search workspace items folder or map, not all descendants
    local map = getMapContent and getMapContent()
    local searchRoot = map or svc.WS
    for _, obj in ipairs(searchRoot:GetChildren()) do
        checkObj(obj)
        for _, child in ipairs(obj:GetChildren()) do checkObj(child) end
    end
    return items
end
