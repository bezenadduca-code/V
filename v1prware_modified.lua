...
do
    ...
    -- Hook 1: RemoteFunction GetMousePosition callback (like Nova)
    local function plasmaPatchRF()
        ...
        rf.OnClientInvoke = function(reqName, data, ...)
            if reqName == "GetMousePosition" and plasma_enabled then
                ...
                if hrp then
                    return hrp.Position + Vector3.new(0, plasma_aimOffset, 0)
                end
            end
    ...
    -- Hook 2: Namecall hook for UseActorAbility with PlasmaBeam buffer check
    local function plasmaPatchNC()
        if plasma_oldNC then return end
        local ok, re = pcall(function()
            return svc.RS:WaitForChild("Modules", 10)
                :WaitForChild("Network", 10)
                :WaitForChild("Network", 10)
        ...
        if not ok or not re then
            warn("[V1PRWARE] Dusekkar: RemoteEvent not found")
            return
        end
        plasma_re = re
        plasma_oldNC = hookmetamethod(game, "__namecall", function(self, ...)
            if not plasma_enabled then return plasma_oldNC(self, ...) end
            local method = getnamecallmethod()
            if method ~= "FireServer" or self ~= re then
                return plasma_oldNC(self, ...)
            end
            local args = { ... }
            local eventName = tostring(args[1])
            ...
            if not isPlasma then return plasma_oldNC(self, ...) end

            -- Resolve target
            local hrp = plasmaGetNearestKiller()
            if not hrp then return plasma_oldNC(self, ...) end

            local char  = lp.Character
            local myHRP = char and char:FindFirstChild("HumanoidRootPart")
            if not myHRP then return plasma_oldNC(self, ...) end
            ...
            return plasma_oldNC(re, "UseActorAbility", {
    ...
    local function plasmaUnpatch()
        ...
        -- Unpatch namecall
        if plasma_oldNC then
            pcall(function()
                hookmetamethod(game, "__namecall", plasma_oldNC)
            end)
            plasma_oldNC = nil
        end
        plasma_re = nil
    ...
    sec_028:Button({
        Title = "Unload PlasmaBeam Hooks", Callback = function()
