...
------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: NOSFERATU (Auto Hook QTE)
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabNosferatu = win:Tab({ Title = "Nosferatu", Icon = "anchor" })

do
    local secNosMain = tabNosferatu:Section({ Title = "Auto Hook QTE", Opened = true })

    secNosMain:Paragraph({
        Title   = "How it works",
        Content = "Hooks the Network FireServerConnection to auto-complete Nosferatu's hook minigame. When the QTE remote fires, the callback answers it immediately at the configured cooldown interval.",
    })

    local nos_enabled  = cfg.get("nosEnabled",  false)
    local nos_cooldown = cfg.get("nosCooldown",  0.3)
    local nos_network  = nil
    local nos_origFire = nil
    local nos_lastFire = 0

    local function nosGetNetwork()
        if nos_network then return nos_network end
        local ok, m = pcall(function()
            return require(svc.RS.Modules.Network.Network)
        end)
        if ok and m then nos_network = m end
        return nos_network
    end

    local function nosPatch()
        local m = nosGetNetwork(); if not m then warn("[V1PRWARE] Nosferatu: Network not found"); return end
        if nos_origFire then return end -- already patched
        nos_origFire = m.FireServerConnection
        m.FireServerConnection = function(self, eventName, eventType, ...)
            -- intercept the hook QTE remote
            if nos_enabled and eventName and string.find(tostring(eventName), "Hook", 1, true) then
                local now = tick()
                if (now - nos_lastFire) >= nos_cooldown then
                    nos_lastFire = now
                    -- call original to actually complete it
                    return nos_origFire(self, eventName, eventType, ...)
                end
                return
            end
            return nos_origFire(self, eventName, eventType, ...)
    ...
    local function nosUnpatch()
        local m = nosGetNetwork(); if not m or not nos_origFire then return end
        m.FireServerConnection = nos_origFire
        nos_origFire = nil
    end

    -- Also hook RenderStepped to auto-complete the visible minigame UI
    local nos_uiConn = nil
    local function nosWatchUI(on)
        if nos_uiConn then nos_uiConn:Disconnect(); nos_uiConn = nil end
        if not on then return end
        nos_uiConn = svc.Run.Heartbeat:Connect(function()
            if not nos_enabled then return end
            -- Look for the hook minigame ScreenGui in PlayerGui
            local pg = lp:FindFirstChild("PlayerGui"); if not pg then return end
            for _, gui in ipairs(pg:GetChildren()) do
                -- Common pattern: a ScreenGui named around "Hook" or "Minigame"
                if gui:IsA("ScreenGui") and (
                    string.find(gui.Name, "Hook",     1, true) or
                    string.find(gui.Name, "Minigame", 1, true) or
                    string.find(gui.Name, "QTE",      1, true)
                ) then
                    -- Find any button/frame that represents the success zone and fire it
                    local btn = gui:FindFirstChildOfClass("TextButton", true)
                           or  gui:FindFirstChildOfClass("ImageButton", true)
                    if btn then
    ...
    secNosMain:Toggle({
        Title = "Enable Auto Hook QTE", Type = "Checkbox", Default = nos_enabled,
        Callback = function(on)
            nos_enabled = on; cfg.set("nosEnabled", on)
            if on then nosPatch() else nosUnpatch() end
            nosWatchUI(on)
        end
    })

    secNosMain:Slider({
        Title = "QTE Cooldown (s)", Value = { Min = 0.1, Max = 1.0, Default = nos_cooldown }, Step = 0.05,
        Callback = function(v) nos_cooldown = v; cfg.set("nosCooldown", v) end
    })

    local secNosCtrl = tabNosferatu:Section({ Title = "Control", Opened = true })
    secNosCtrl:Button({
        Title = "Unpatch Hook", Callback = function()
            nos_enabled = false; nosUnpatch(); nosWatchUI(false)
        end
    })
end
...
------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: VEERONICA GRAFFITI ESP
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabGraffiti = win:Tab({ Title = "Veeronica", Icon = "spray-can" })

do
    local secGraffiti = tabGraffiti:Section({ Title = "Graffiti ESP", Opened = true })

    local graffiti_enabled = cfg.get("graffitiEnabled", false)
    local graffiti_color   = cfg.get("graffitiColor",   Color3.fromRGB(255, 192, 203))
    local graffiti_conn    = nil
    local graffiti_draws   = {} -- model -> { highlight, label }

    local function graffitiCleanup(model)
        local d = graffiti_draws[model]
        if d then
            pcall(function() d.highlight:Destroy() end)
            pcall(function() d.label:Remove() end)
            graffiti_draws[model] = nil
        end
    end

    local function graffitiAdd(model)
        if graffiti_draws[model] then return end
        -- Highlight
        local hl = Instance.new("Highlight")
        hl.FillColor       = graffiti_color
        hl.OutlineColor    = graffiti_color
        hl.FillTransparency    = 0.5
        hl.OutlineTransparency = 0
        hl.Parent = model

        -- Distance label via Drawing
        ...
        label.Font    = 2
        label.Size    = 15
        label.Color   = graffiti_color

        graffiti_draws[model] = { highlight = hl, label = label }
    end

    local function graffitiScanAll()
        local ig = getIngame(); if not ig then return end
        for _, v in ipairs(ig:GetChildren()) do
            if v.Name == "GraffitiCL" then
                if graffiti_enabled then graffitiAdd(v) else graffitiCleanup(v) end
            end
        end
    end

    local function graffitiStart()
        if graffiti_conn then return end
        graffitiScanAll()

        graffiti_conn = svc.Run.RenderStepped:Connect(function()
            if not graffiti_enabled then return end
            local cam = workspace.CurrentCamera
            local char = lp.Character
            local myHRP = char and char:FindFirstChild("HumanoidRootPart")

            for model, d in pairs(graffiti_draws) do
                if not model or not model.Parent then
                    graffitiCleanup(model)
                else
                    local part = model:FindFirstChildOfClass("BasePart")
                    if part and myHRP then
                        local dist = (part.Position - myHRP.Position).Magnitude
                        local screenPos, onScreen = cam:WorldToViewportPoint(part.Position)
                        d.label.Visible  = onScreen
                        d.label.Position = Vector2.new(screenPos.X, screenPos.Y - 20)
                        d.label.Text     = string.format("Graffiti  %.1f studs", dist)
                        d.label.Color    = graffiti_color
                        d.highlight.FillColor    = graffiti_color
                        d.highlight.OutlineColor = graffiti_color
                    else
        ...
        -- Watch for new graffiti spawning mid-match
        local ig = getIngame()
        if ig then
            ig.ChildAdded:Connect(function(child)
                if child.Name == "GraffitiCL" and graffiti_enabled then
                    task.wait(0.1)
                    graffitiAdd(child)
                end
            end)
            ig.ChildRemoved:Connect(function(child)
                if child.Name == "GraffitiCL" then
                    graffitiCleanup(child)
                end
            end)
        end
    end

    local function graffitiStop()
        if graffiti_conn then graffiti_conn:Disconnect(); graffiti_conn = nil end
        for model, _ in pairs(graffiti_draws) do graffitiCleanup(model) end
        graffiti_draws = {}
    end

    secGraffiti:Toggle({
        Title = "Graffiti ESP", Type = "Checkbox", Default = graffiti_enabled,
        Callback = function(on)
            graffiti_enabled = on; cfg.set("graffitiEnabled", on)
            if on then graffitiStart() else graffitiStop() end
        end
    })

    secGraffiti:ColorPicker({
        Title = "Graffiti Color", Default = graffiti_color,
        Callback = function(c)
            graffiti_color = c; cfg.set("graffitiColor", c)
        end
    })

    local secGraffitiCtrl = tabGraffiti:Section({ Title = "Control", Opened = true })
    secGraffitiCtrl:Button({
        Title = "Refresh Scan", Callback = function()
            graffitiScanAll()
        end
    })
end
